...
export function PersianDateRangePicker(props: PersianDateRangePickerProps) {
  ...
  const [focusedDate, setFocusedDate] = React.useState<Date>(() => value.start ?? value.end ?? getToday())
  const [hoverDate, setHoverDate] = React.useState<Date | null>(null)

  const [textStart, setTextStart] = React.useState<string>(() => (value.start ? formatValue(value.start) : ''))
  const [textEnd, setTextEnd] = React.useState<string>(() => (value.end ? formatValue(value.end) : ''))
  ...
  }, [formatValue, value.end, value.start])
  ...
  React.useEffect(() => {
    ...
    setViewMonth((m) => (m.jy === p.jy && m.jm === p.jm ? m : { jy: p.jy, jm: p.jm }))
  }, [focusedDate, open])
  ...
  const grid = React.useMemo(() => buildJalaliMonthGrid({ jy: viewMonth.jy, jm: viewMonth.jm }), [viewMonth.jm, viewMonth.jy])

  function openCalendar(field: 'start' | 'end') {
    if (disabled) return
    setActiveField(field)
    setOpen(true)
    const anchor = (field === 'start' ? value.start : value.end) ?? value.start ?? value.end ?? getToday()
  ...
  function closeCalendar() {
    ...
    if (mode === 'popover') {
      ...
      else (activeField === 'start' ? inputStartRef : inputEndRef).current?.focus()
    }
  }
  ...
  function navigateMonth(delta: -1 | 1) {
    const anchor = fromJalaliParts({ jy: viewMonth.jy, jm: viewMonth.jm, jd: 1 })
  ...
  function getRangeForJalaliYear(jy: number): { start: Date; endExclusive: Date } {
    const start = fromJalaliParts({ jy, jm: 1, jd: 1 })
    const endExclusive = fromJalaliParts({ jy: jy + 1, jm: 1, jd: 1 })
    return { start, endExclusive }
  }

  function getRangeForJalaliMonth(jy: number, jm: number): { start: Date; endExclusive: Date } {
  ...
  function rangeIntersectsPickerLimits(range: { start: Date; endExclusive: Date }) {
  ...
  function selectMonth(jm: number) {
    ...
    setFocusedDate(() => fromJalaliParts({ jy: nextView.jy, jm: nextView.jm, jd: 1 }))
    setPanel('days')
  }
  ...
  function cellClass(cell: CalendarDayCell) {
    const base = cx(
      ...
      isCellDisabled(cell) && cx('dvx-pdp__day--disabled', classes?.dayDisabled),
      isSameDay(cell.gregorian, getToday()) && cx('dvx-pdp__day--today', classes?.dayToday),
    )
    ...
    if (start && endOrPreview && (value.end || showPreview) && isBetweenInclusive(cell.gregorian, start, endOrPreview)) {
  ...
  const calendarUi = (
    <div ref={calendarRef} tabIndex={-1} className="dvx-pdp__calendar" onKeyDown={onCalendarKeyDown}>
      <div className={cx('dvx-pdp__header', classes?.header)}>
        <button
          type="button"
          className={cx('dvx-pdp__nav', classes?.navButton)}
          onClick={panel === 'days' ? () => navigateMonth(-1) : () => setYearPageStart((s) => s - 12)}
      ...
      {panel === 'days' ? (
        <div className={cx('dvx-pdp__grid', classes?.grid)}>
          <div className="dvx-pdp__weekdays" aria-hidden="true">
            {weekdayLabels.map((w, idx) => (
              <div key={idx} className={cx('dvx-pdp__weekday', classes?.weekday)}>
                {w}
              </div>
            ))}
          </div>
                ...
                {row.map((cell) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      className={cx(cellClass(cell), isFocused && 'dvx-pdp__day--focused')}
                      ...
                      onMouseEnter={() => {
                        ...
                        if (value.start && !value.end) setHoverDate(cell.gregorian)
                      }}
        ...
        <div className="dvx-pdp__panel">
          <div className="dvx-pdp__panelGrid">
            {Array.from({ length: 12 }).map((_, i) => {
              ...
              const enabled = rangeIntersectsPickerLimits(getRangeForJalaliYear(jy))
              const isActive = jy === viewMonth.jy
              return (
                <button
                  key={jy}
                  type="button"
                  className={cx('dvx-pdp__cell', isActive && 'dvx-pdp__cell--active')}
                  disabled={!enabled}
        ...
        <div className="dvx-pdp__panel">
          <div className="dvx-pdp__panelGrid">
            {Array.from({ length: 12 }).map((_, i) => {
              const jm = i + 1
              const enabled = rangeIntersectsPickerLimits(getRangeForJalaliMonth(pendingYear, jm))
              const isActive = pendingYear === viewMonth.jy && jm === viewMonth.jm
              const monthText = monthLabels && monthLabels.length === 12 ? (monthLabels[jm - 1] ?? String(jm)) : String(jm)
              return (
                <button
                  key={jm}
                  type="button"
  ...
  return (
    <div ref={rootRef} className={cx('dvx-pdp', mode === 'inline' && 'dvx-pdp--inline', className, classes?.root)} dir="rtl">
      {mode === 'inline' ? null : (
      ...
      {mode === 'inline' ? (
        ...
        <div className={cx('dvx-pdp__popover', classes?.popover)} role="dialog" aria-modal="false">
          {calendarUi}
        </div>
      ) : null}
    </div>
