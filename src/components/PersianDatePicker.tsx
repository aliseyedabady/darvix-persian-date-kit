...
function normalizeInputValue(next: Date, minDate?: Date, maxDate?: Date): Date | null {
...
export function PersianDatePicker(props: PersianDatePickerProps) {
  ...
  const [viewMonth, setViewMonth] = React.useState(() => getInitialViewMonth(value))
  const [focusedDate, setFocusedDate] = React.useState<Date>(() => {
    const base = value ?? getToday()
    return base
  })
  const [text, setText] = React.useState<string>(() => (value ? formatValue(value) : ''))
  ...
  const [pendingYear, setPendingYear] = React.useState<number>(() => getInitialViewMonth(value).jy)
  ...
  function navigateMonth(delta: -1 | 1) {
    ...
    setFocusedDate((curr) => {
      ...
      const currIsInView = currParts.jy === viewMonth.jy && currParts.jm === viewMonth.jm
  ...
  function selectMonth(jm: number) {
    ...
    setFocusedDate(() => {
      ...
      try {
        return fromJalaliParts({ jy: nextView.jy, jm: nextView.jm, jd: currentDay })
      } catch {
    ...
    >
      <div className={cx('dvx-pdp__header', classes?.header)}>
        <button
          ...
          onClick={panel === 'days' ? goPrevMonth : () => setYearPageStart((s) => s - 12)}
        ...
        <button
          type="button"
          className={cx('dvx-pdp__nav', classes?.navButton)}
          onClick={panel === 'days' ? goNextMonth : () => setYearPageStart((s) => s + 12)}
            ...
            {grid.map((row, r) => (
              <div className="dvx-pdp__row" key={r}>
                {row.map((cell) => {
                  ...
                  const isSelected = selected ? isSameDay(cell.gregorian, selected) : false
                  ...
                  return (
                    <button
                      ...
                      className={cx(
                        ...
                        cellDisabled && cx('dvx-pdp__day--disabled', classes?.dayDisabled),
                        isSelected && cx('dvx-pdp__day--selected', classes?.daySelected),
  ...
  return (
    <div
      ref={rootRef}
      className={cx('dvx-pdp', mode === 'inline' && 'dvx-pdp--inline', className, classes?.root)}
      dir="rtl"
    >
      {mode === 'inline' ? null : (
